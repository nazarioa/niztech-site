...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    if ($('#ewineryscript') == null) {
      alert(
        'eWinery is not setup correctly. Please include \' id = "ewineryscript"\' to the ewinery <script></script> tag .',
      );
    } else {
      scriptPath =
        $('#ewineryscript')
    ...
    DEBUG == true
      ? console.log('eWinery: using eWineryFeedURL: ' + eWineryFeedURL)
      : false;
    DEBUG == true
      ? console.log('eWinery: using eWineryStoreURL: ' + eWineryStoreURL)
      : false;
    ...
    cart +=
      '<div>Items: <span class="total-item-count">0</span></div><div>Subtotal: <span class="sub-total">$0.00</span><div class="checkout-wrap"><a class="checkout" href="' +
      eWineryStoreURL +
      '/index.cfm?method=storecart.showcart" >Checkout</a></div></div>';
    cart += '</div>';
    ...
    cart +=
      ...
      eWineryStoreURL +
      '/index.cfm?method=storecart.showcart">Checkout</a>';
    cart += '</div>';
    cart += '-->';
    cart += '</div>';
  ...
  function renderMiniCart() {
    ...
    $('#cart-content p.total span.count').text(
      cartCookieData.length + ' items in cart',
    );
    if (itemsCount != 0) {
      $.each(cartCookieData, function (pr) {
        ...
        if (productData[prID] && cartCookieData[pr].QUANTITY > 0) {
          ...
          i +=
            ...
            productData[prID].PRODUCTNAME +
            '</span><a href="#' +
            prID +
            '" class="remove">&nbsp;</a></p>';
          i +=
            '<p>Price: <span class="product-price">$' +
            parseFloat(cartCookieData[pr].PRICE).toFixed(2) +
            '</span></p>';
          i +=
            '<div>Qty: <span class="product-qty">' +
            cartCookieData[pr].QUANTITY +
            '</span></div>';
          i +=
            '<div>$<span class="product-qty-time-price">' +
            parseFloat(
              cartCookieData[pr].PRICE * cartCookieData[pr].QUANTITY,
            ).toFixed(2) +
            '</span></div>';
          i += '</li>';
    ...
    $('#cart-content ul').append(
      '<li id="total-line"><div><span class="total-item-count"></span> items in cart</div><div>Subtotal: <span class="sub-total"></span></div></li>',
    );
  ...
  function renderProductInfo() {
    DEBUG == true
      ? console.log('eWinery: Entering renderProductInfo()')
      : false;
    ...
    $.each($('.ewinery.ewbutton'), function populateProductBuyButton() {
      ...
      if (productData[buyButtonId] != null) {
        ...
        if (Number(productData[buyButtonId].INVENTORY > 0)) {
          ...
          DEBUG == true
            ? console.log(
                '(Warning) eWinery: wine out of stock: ' + buyButtonId,
              )
            : false;
          $(this).replaceWith(
            '<span id=' +
              productData[buyButtonId].PRODUCTID +
              ' class="ewinery buynow"><img src="' +
              scriptPath +
              'img/buy-now.png"" /></span>',
          );
        } else {
          // out of stock
          DEBUG == true
            ...
            : false;
          $(this).replaceWith(
            '<span class="ewinery soldout"><img src="' +
              scriptPath +
              'img/sold-out.png"" /></span>',
          );
        }
      } else {
        DEBUG == true
          ? console.log(
              '(Warning) eWinery: No wine information found for: ' +
                buyButtonId,
            )
          : false;
        $(this).replaceWith(
          '<span class="ewinery noinfo"><img src="' +
            scriptPath +
            'img/sold-out.png"" /></span>',
        );
      }
    ...
    $.each($('.ewinery.ewprice'), function populateProductPrice() {
      ...
      if (productData[productPriceId] != null) {
        $(this).replaceWith(
          'Price: $' +
            parseFloat(productData[productPriceId].PRICE1).toFixed(2),
        );
      } else {
        $(this).replaceWith('');
      }
  ...
  function attachButtonsToClick() {
    DEBUG == true
      ? console.log('eWinery: Entering attachButtonsToClick()')
      : false;
    ...
    for (var rr = 0; rr < cartCookieData.length; rr++) {
      ...
      if (cartCookieData[rr].PRODUCTID == clickedProductId) {
        if (cartCookieData[rr].QUANTITY > numberToRemove) {
          var newQtyForThisProduct =
            Number(cartCookieData[rr].QUANTITY) - numberToRemove;
          ...
          DEBUG == true
            ? console.log("eWinery: removed item: '" + clickedProductId + "'")
            : false;
        }
      }
    }
  ...
  function addItem(clickedProductId, numberToAdd) {
    DEBUG == true
      ? console.log("eWinery: clicked on: '" + clickedProductId + "'")
      : false;
  ...
  function getSubTotal() {
    ...
    $.each(cartCookieData, function (nu) {
      subTotal +=
        Number(cartCookieData[nu].QUANTITY) * Number(cartCookieData[nu].PRICE);
    });
